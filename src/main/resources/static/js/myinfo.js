	...
	createCouponListForm() {
		...
		fetch("/api/myCoupon", {
			...
			for (let i = 0; i < data.length; i++) {
				...
				const childNodeArray2 = [nameDivTag, contentDivTag, dateDivTag];
				childNodeArray2.forEach((node) => {
					couponInfoDivTag.append(node);
				});
				
				const childNodeArray3 = [startDateDivTag, endDateDivTag];
				childNodeArray3.forEach((node) => {
					dateDivTag.append(node);
				});
